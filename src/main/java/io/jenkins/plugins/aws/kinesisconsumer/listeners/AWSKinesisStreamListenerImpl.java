...
public class AWSKinesisStreamListenerImpl extends AWSKinesisStreamListener {
    ...
    public void onReceive(String streamName, byte[] bytes) {
        ...
        // config
        getProjectFromEvent(streamName, bytes)
                .ifPresent(
                    projectEvent -> {
                        ...
                        try (ACLContext acl = ACL.as(ACL.SYSTEM)) {
                            ...
                            for (SCMSourceOwner scmJob : jenkinsItems) {
                                ...
                                for (SCMSource scmSource : scmSources) {
                                    if(projectEvent.equals(getProjectFromRemote(scmSource))) {
    ...
    Optional <String> getProjectFromEvent(String streamName, byte[] bytes) {
        return Optional.ofNullable(getProjectField(streamName)).flatMap( projectField -> {
            ObjectMapper jsonMapper = new ObjectMapper();
            try {
                Map <String, String> stringStringMap =
                        jsonMapper.readValue(new String(bytes, StandardCharsets.UTF_8),
    ...
    Optional<String> getProjectField(String streamName) {
        GlobalKinesisConfiguration globalKinesisConfiguration =
        GlobalKinesisConfiguration.get();

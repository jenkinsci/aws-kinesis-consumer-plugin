    ...
    public static class DescriptorImpl extends Descriptor<KinesisStreamItem> {
        ...
        public FormValidation doTestConnection(@QueryParameter("region") final String region,
            ...
            try {
                ...
                DescribeStreamResponse describeStream = kinesisAsyncClient.describeStream(DescribeStreamRequest.builder().streamName(streamName).build()).get();
                ...
                if (Arrays.asList(CREATING, ACTIVE, UPDATING).contains(streamStatus)) {
                    return FormValidation.ok("Success: stream %s (arn: %s). Status %s", streamName, streamARN, streamStatus);
                }
                return FormValidation.error("Failure: stream %s (arn: %s). Status %s", streamName, streamARN, streamStatus);
        ...
        private KinesisAsyncClient kinesisAsyncClient(String region, String localEndpoint) throws URISyntaxException {

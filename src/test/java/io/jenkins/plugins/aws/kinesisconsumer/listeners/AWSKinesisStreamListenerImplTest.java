...
public class AWSKinesisStreamListenerImplTest {
  ...
  public void shouldTriggerSCMBuild() throws Exception {
    ...
    try (MockedStatic<SCMSourceEvent> dummy = Mockito.mockStatic(SCMSourceEvent.class)) {
      dummy.when(() -> SCMSourceEvent.fireNow(Mockito.any())).thenCallRealMethod();

      new AWSKinesisStreamListenerImpl()
          .onReceive(streamName, "{\"projectField\":\"" + projectName + "\"}");
      j.waitUntilNoActivity();

      dummy.verify(() -> SCMSourceEvent.fireNow(Mockito.any()));
    }
  }
